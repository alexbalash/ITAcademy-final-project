...
createDate = function(view){ 
    ...
    if (startDay != 1) {
      ...
      for (var i=1; i<blanks; i++) { cells.push("b"); }
    }
    for (var i=1; i<=daysInMonth; i++) { cells.push(i); }
    if (endDay != 0) {
        var blanks = endDay==6 ? 1 : 7-endDay;
        for (var i=0; i<blanks; i++) { cells.push("b"); }
    }
        return cells
  }
  ...
  updateServerStorage = function(pswd,view) {
	...
	var self = this;
		$.ajax({
			...
			data: { f : 'UPDATE', n : handle, v : JSON.stringify(view.calendar.data), p : pswd },
			...
			success: function (data) {
				if (self.handleErrors(data.error) ) {
					if ("" == data.result) {
						$.ajax({
							url: url, type : 'POST', dataType:'json', cache : false,
							data: { f : 'INSERT', n : handle, v : JSON.stringify(view.calendar.data) }, success: self.handleSuccess
						});
					} else {
	...
	var handle = 'altushka';
		...
		$.ajax({
			...
			success: function (data) {
				if ( self.handleErrors(data.error) ) {
					view.calendar.data = JSON.parse(data.result);
					console.log(view.calendar.data)
